...
func (o *Options) waitForGitOpsPullRequest(ns string, env *jxcore.EnvironmentConfig, releaseInfo *ReleaseInfo, end time.Time, duration time.Duration, promoteKey *activities.PromoteStepActivityKey) error {
	prInfo := releaseInfo.PullRequestInfo
	if prInfo == nil {
		return nil
	}
...
func (o *Options) checkMergePullRequest(ctx context.Context, repo scm.Repository, pr *scm.PullRequest, prInfo *scm.PullRequest, logMergeFailure bool) {
	if !o.NoMergePullRequest { // Is automatic merge of promote Pull Requests disabled? (default disabled = false)
		...
		if !tideMerge {
			...
			_, err = o.ScmClient.PullRequests.Merge(ctx, repo.FullName, prInfo.Number, prMergeOptions)
			// TODO
...
func (o *Options) completePromotion(ns string, env *jxcore.EnvironmentConfig, pr *scm.PullRequest, promoteKey *activities.PromoteStepActivityKey) error {
